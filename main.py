...
def regio(weight_a: float = 1/3, weight_b: float = 1/3, weight_c: float = 1/3):
    ...
    proc_times_r_2 = robust_scaling(np.array([proc(i) for i in times_r_2]), True)
    # ic(proc_times_r_2)
    # ic(arbit_minmax_normalize(np.array([proc(i) for i in times_r_2]), (0, 2)))


    weighted_r = r_w * proc_times_r_2
    r_price = robust_scaling(r, inv=True)
    # weighted_r = proc_times_r_2
    # ic(weighted_r)
    # ic(r_price)
    ic(score)
    return zip(times_r, weight_a * weighted_r, weight_b * r_price, weight_c * score, r)
...
def leo(weight_a: float = 1/3, weight_b: float = 1/3, weight_c: float = 1/3):
    ...
    weighted_l = l_w * proc_times_l_2
    l_price = zscore(l * (1 - 0.25))
    ic(l_price)
    ic(l)
    # print(l * (1 - 0.25))
    ic(weighted_l, score, weighted_l + score)
    return zip(times_l, weight_a * weighted_l, weight_b * l_price, weight_c * score, l)
...
def main():
    ...
    result = regio(*weight)
    print("Regiojet")
    for t, a, b, c, price in result:
        h, m = t
        final_score = round(a + b + c, 4)
        print('\t', f"{h}:{m}", price, round(a, 4), round(b, 4), round(c, 4), round(a + b + c, 4))
        if final_score > max_r:
            max_r = final_score
            max_r_price = price
    r2 = leo(*weight)
    print("Leo")
    for t, a, b, c, price in r2:
        h, m = t
        final_score = round(a + b + c, 4)
        print('\t', f"{h}:{m}", price, round(a, 4), round(b, 4), round(c, 4), round(a + b + c, 4))
    ...
    print(max_r, max_l)
    print(max_r_price * forecast, max_l_price * (1 - 0.25) * forecast)
